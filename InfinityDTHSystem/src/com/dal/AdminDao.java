...
private static String tableDebt = "NEW_NEON_CUST_DEBT";
	...
	public int insertRetailer (Retailer retailer){
		...
		public Retailer getRetailerById(int id){
			...
			try{
				// Prepare Query
				String qry = String.format("SELECT * FROM %s WHERE retailer_id = ?", tableRetailer);
				PreparedStatement pst = con.prepareStatement(qry);
				pst.setInt(1, id);
		...
		public int updateRetailerById(int id, Retailer retailer){
			...
			try{
				...
				String qry = "UPDATE NEW_NEON_RETAILER SET RETAILER_NAME = ?, RETAILER_CONTACT1 = ?, RETAILER_CONTACT2 = ?, ADDRESS1 = ?, ADDRESS2 = ?, ZIP_CODE = ?, CITY = ?, STATE_PROVINCE = ?, RETAILER_BOX_LIMIT = ?, RETAILER_CREDIT_LIMIT = ?, RETAILER_COMMISSION = ?, RETAILER_SERVICE_CHARGE = ?, RETAILER_INVENTORY_ID = ?, RETAILER_INVENTORY_COST = ?, WHERE RETAILER_ID = ?";
				
				PreparedStatement pst = con.prepareStatement(qry);
				pst.setString(1, retailer.getRetailName());
				pst.setString(2, retailer.getRetailContact1());
				pst.setString(3, retailer.getRetailContact2());
				pst.setString(4, retailer.getRetailAddress1());
				pst.setString(5, retailer.getRetailAddress2());
				pst.setInt(6, retailer.getRetailZip());
				pst.setString(7, retailer.getRetailCity());
				pst.setString(8, retailer.getRetailState());
				pst.setInt(9, retailer.getBoxLimit());
				pst.setInt(10, retailer.getCreditLimit());
				pst.setInt(11, retailer.getRetailCommission());
				pst.setInt(12, retailer.getRetailServiceCharge());
				pst.setInt(13, retailer.getRetailInventId());
				pst.setInt(14, retailer.getInventCost());
				pst.setInt(15, retailer.getRetailerId());
